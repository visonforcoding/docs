...
安装Cakephp很容易很简单. 一个web服务器再复制一份cakephp就可以了, 就这么简单! 不过这一章的重点在于
如何将cakephp运行在apache上(因为apache很容易安装和运行), CakePHP 可以运行在很多web服务器上
例如 nginx, LightHTTPD, 还有Microsoft IIS.

要求
============

- HTTP 服务器. 例如: Apache. 开启了rewrite 模块更好，如果没开也可以
- PHP 5.5.9 或更高版本 (包括 PHP 7).
- PHP开启mbstring 拓展
- php intl 拓展

.. note::
    XAMPP 和 WAMP, mbstring 拓展默认就已经开启了.XAMPP里面, intl 拓展已经有了，只需要在**php.ini**里把
    ``extension=php_intl.dll`` 这行注释打开， 然后在Xampp 面板里重启服务器。
   WAMP里，intl拓展默认处于激活状态，但是并没有正常工作。你需要去对应版本的php目录里复制出所有
   像**icu.dll**这样的文件到对应版本的apache的bin目录里，然后重启服务器。
